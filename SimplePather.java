...
public class SimplePather {
	...
	public SimplePather(RobotController rc) {
		...
		q = new MapLocation[8000];
		costs = new float[8000];
	...
	private void check(MapLocation parent, int dir) throws GameActionException {
		...
		if (prev[nx][ny] == 0 || cost[nx][ny] > potentialCost) {
			add(n, n.distanceSquaredTo(dest) * ((rc.senseTerrainTile(n)==TerrainTile.UNKNOWN)?5f:1f) + potentialCost);
			prev[nx][ny] = dir;
			cost[nx][ny] = potentialCost;
		}
	}

	public boolean invalid(MapLocation offset) {
		return (offset.x >= maxX || offset.x < 0 || offset.y >= maxY || offset.y < 0);
	}
